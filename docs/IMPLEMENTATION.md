...
src/server/
├── services/
│   ├── football-api.ts       # API-Football 클라이언트
...
| api_fixture_id | INTEGER | API-Football fixture id |
...
| `API_FOOTBALL_KEY` | 2 | API-Football |
| `KAKAO_*` | 3 | 카카오 로그인·메시지 |
| `VAPID_*` | 4 | Web Push |
| `SESSION_SECRET`, `TOKEN_ENCRYPTION_KEY` | 3 | 세션·토큰 암호화 |
...
# .env 에 API_FOOTBALL_KEY 입력
npm run verify:fixtures  # 6경기 fixture ID·팀명 검증 (API 1회/경기일)
...
| 항목 | 설명 |
|------|------|
| `npm run setup:env` | `.env` + 랜덤 `SESSION_SECRET` / `ADMIN_SECRET` / `TOKEN_ENCRYPTION_KEY` |
| `npm run verify:fixtures` | API로 6경기 fixture 조회·팀명 검증 |
| `railway.toml` | Railway 빌드·시작 명령 |
| `.nvmrc` | Node 22 |

### 당신이 직접 해야 할 것

1. **`.env`에 `API_FOOTBALL_KEY` 입력** → `npm run verify:fixtures` (6/6 OK 확인)
...
4. **Railway** (6/26 전)
   ...
   - Variables: `API_FOOTBALL_KEY`, `ADMIN_SECRET`, `DATABASE_PATH=/data/app.db`, `PORT=3000`
   - Volume `/data` 마운트, RAM 256MB
   - Deploy 로그에서 fixture 6경기 OK 확인
5. **6/26 저녁 배포 · 6/28 이후 서비스 중지**

### API-Football Free 플랜 제한 (중요)

실제 API 응답:

> `Free plans do not have access to this season, try from 2022 to 2024.`

**2026 월드컵(`season=2026`) 데이터는 Free 플랜에서 조회 불가**합니다. 이 상태에서는:

- `verify:fixtures` 6/6 실패
- 자동 폴링도 fixture ID를 못 받아 **동작하지 않음**
- **대시보드 + `/admin` 수동 스코어**는 그대로 사용 가능

**선택지**

| 선택 | 내용 |
|------|------|
| **A) API 유료 플랜** | [api-football.com/pricing](https://www.api-football.com/pricing) Pro 등 업그레이드 후 `verify:fixtures` 재실행 |
| **B) 수동만** | 경기 끝날 때마다 `/admin`에서 스코어 입력 (API 비용 $0) |
